...
const HeaderCard: FC<HeaderCardProps> = ({
  ...
  const [orientationClass, setOrientationClass] = useState(
    orientation == 'portrait' ? 'vertical-rl-all' : ''
  )
  return (
    <span
      onClick={() => {
      ...
      className={
        orientationClass +
        ' flex flex-row cursor-pointer font-bold justify-between p-5 border border-4 border-black items-center z-50 hover:bg-[#FCF2DC] ' +
        className
      }
    >
