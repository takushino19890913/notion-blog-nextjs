...
const renderBlock = (block) => {
  ...
  switch (type) {
    ...
    case 'code':
      ...
      return (
        <CodeBlock
          ...
          codeString={value.rich_text[0].text.content}
        />
      )
    case 'file':
      const src_file =
    ...
    case 'bookmark':
      ...
      return <MetaCard url={href} metaData={block.metaData} />
    case 'table': {
      return (
    ...
    case 'embed': {
      if (hasTwitterUrl(value.url)) {
        return <EmbedTwitter text={value.url} />
      }
    }
    default:
...
export default function Post({ page, blocks }) {
  ...
  return (
    <div>
      ...
      <Layout>
        <article
          className="px-8 md:px-5 mx-auto whitespace-pre-line"
          style={{ maxWidth: '700px', lineHeight: '1.5' }}
        >
          <h1 className="my-10 text-4xl md:text-5xl">
...
export const getStaticProps = async (context) => {
  ...
  // ブックマークのメタデータを取得する
  const blocksWithMetaData = await Promise.all(
