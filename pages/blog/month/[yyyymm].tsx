...
import { getDatabase, getDatabaseForMonth, getDatabaseWithTimeStamp } from "../../../lib/notion";
import { Text } from "../[id]";
...
export default function Blog({ posts, currentMonth, year_month_list, tag_list }) {
    ...
    return (
      <div>
        ...
        <div className="ml-40 w-1/6">
            ...
            <div className="my-10">
            <SearchSelect name="monthSelect"  labelTitle="月を選択" optionList={year_month_list} selected={currentMonth.slice(0,4)+ currentMonth.slice(4)} pathname="/blog"/>
            </div>
            <div>
          ...
          <h2 className="text-3xl pb-5 border-b-2 border-dotted border-black">{currentMonth}</h2>
          <ol className="mt-5">
...
export const getStaticPaths = async () => {
    ...
    const paths = Array.from(dates).map(date => ({ params: { yyyymm: 
...
export const getStaticProps = async (context) => {
    ...
    const database: Page[] = await getDatabaseForMonth(databaseId, yyyymm) as unknown as Page[];
    const dates = new Set<Date>();
  ...
  await Promise.all(
    databaseAll.map(async (item) => {
      ...
      if (year_month_list[year_month]) {
        ...
        year_month_list[year_month] = new Array(
          item
        )
      }
      await Promise.all(
